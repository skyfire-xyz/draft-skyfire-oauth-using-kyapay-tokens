...
This document describes how security intermediaries -- bot managers, fraud managers, account-takeover (ATO) protection systems, and customer identity and access management (CIAM) systems -- consume KYAPay tokens to answer a question that traditional bot detection cannot: not "is this a human or a bot?", but "did a verified human authorize this agent?".
It specifies how KYAPay tokens are carried in HTTP requests, how they are validated (including in combination with request-signing layers such as HTTP Message Signatures), and how the verified, layered identity in a token is used to make access, routing, fraud, account-lifecycle, and step-up decisions.
...
And the sharper form of the question is not "is this a human or a bot?" but "did a verified human authorize this agent?".
...
A human-via- agent request is, at the transport and application layers, frequently indistinguishable from a bot: it is programmatic, it may originate from data-center IP space, and it may not carry a conventional interactive browser fingerprint.
...
This document describes how *consumers* of KYAPay tokens -- in particular the security intermediaries that guard websites, APIs, and accounts -- carry, validate, and act on KYAPay tokens.
...
Two related specifications define JWT claims and values that a verifier can use to reason about how a principal was authenticated and verified, and that MAY appear in KYAPay tokens: the Additional Authentication Method Reference Values specification {{I-D.skyfire-oauth-amr-values}}, which extends the `amr` claim {{RFC8176}}, and the Identity Verification Methods Values specification {{I-D.skyfire-oauth-id-verification}}, which defines the `ivm` claim.
...
The Initiator/Target and agent- identity terms (Initiator Agent, Initiator Principal, Agent Platform, and the identity groupings `hid`, `apd`, and `aid`) are defined in {{I-D.skyfire-oauth-kyapay-token}} and are used here with the same meaning.
...
: The entity that receives a KYAPay token, validates it, and acts on the result.
  ...
  This role is not defined by {{I-D.skyfire-oauth-kyapay-token}};
  it is defined by this document.
...
KYAPay tokens are structured to carry these identities in layers (`hid`, `apd`, and `aid`) so that this auditable, cryptographically verifiable chain of responsibility -- from agent action back to an accountable human -- can be established.
...
KYAPay meets this by carrying credentials in the HTTP request itself ({{CarryingRequests}}), which requires no change to how a merchant's site is built.
...
Its single most valuable operational property is that a Target has to do almost nothing to consume it: verify a JWT against the issuer's public key via JWKS, exactly as it would verify any OpenID Connect or OAuth token.
...
* *Did the sender actually hold the credential's key?*
  ...
  over HTTP, HTTP Message Signatures {{RFC9421}} are the mechanism for this.
  A signature proves control of a key but says nothing about whose key it is -- it is a *verb*, not an identity format.

Because these are different layers, HTTP Message Signatures {{RFC9421}} and similar request-signing mechanisms are complementary to KYA, not alternatives to it: request signing supplies authenticity and possession, while KYA supplies identity, authorization, and payment.
...
# Carrying KYAPay Tokens in Requests {#CarryingRequests}
...
The `KYAPay-Token` header field is a request header field whose value is the JWS Compact Serialization {{RFC7515}} of a KYA, PAY, or KYA-PAY token as defined in {{I-D.skyfire-oauth-kyapay-token}}.
HTTP field names are case-insensitive {{RFC9110}};
early deployments write this field in lowercase as `kyapay-token`.

    KYAPay-Token = token-jws *( OWS "," OWS token-jws )
    token-jws    = 1*( ALPHA / DIGIT / "-" / "_" / "." )

A request MAY carry more than one token (for example, a separate KYA token and PAY token) either as a comma-separated list in a single `KYAPay-Token` header field or as multiple `KYAPay-Token` header fields;
per {{Section 5.2 of RFC9110}}, multiple field lines with the same name are equivalent to a single comma-separated field line.
Because the JWS Compact Serialization uses only the base64url alphabet plus the period (".") separator, it never contains a comma, so list parsing is unambiguous.

A sender:

* MUST place exactly one JWS Compact Serialization in each list member;
...
* MUST ignore a `KYAPay-Token` member it cannot parse as a JWS, while continuing to process the remaining members, unless local policy requires rejecting the whole request.
...
Where the token carries a proof-of-possession key ({{TrustStack}}), it SHOULD be accompanied by a request Message Signature {{RFC9421}} over that key;
otherwise it is a bearer token, subject to the controls and accepted risks in {{SecCon}}.

## Carrying Tokens over Other Interfaces
...
Carrying a token securely also requires that the sending agent deliver it to the intended recipient and that the recipient be able to bind the token to itself.
...
1. MUST extract each token as described in {{CarryingRequests}}.
...
1. MUST confirm that the `env` claim (for example, `production`) matches the environment the intermediary is operating in.
   Where the token carries a proof-of-possession key (the `cnf` claim {{RFC7800}}, Section 4), MUST verify that the request is signed by that key -- over HTTP, using HTTP Message Signatures {{RFC9421}} -- and reject the request if the signature is absent or invalid.
1. Where the token carries no such key, it is a bearer token: the intermediary MUST rely on the controls in {{SecCon}} (short lifetime, audience binding, TLS, issuer-gated onboarding) and MUST NOT assume that mere presentation of the token proves the sender holds a key bound to it.
1. Only after successful validation, SHOULD use the token's claims to inform its decision as described in the following subsections.
...
* the human principal (`hid`): the presence and content of the human identity claim, and how strongly it was verified -- for example a verified status and verifier ({{I-D.skyfire-oauth-kyapay-token}}), and, where a deployment conveys it, an identity-proofing assurance level (such as an Identity Assurance Level per {{NIST-800-63A}}) and the `amr` {{I-D.skyfire-oauth-amr-values}} and `ivm` {{I-D.skyfire-oauth-id-verification}} claims describing how the principal was authenticated and verified;
...
* **Low Latency.**
  Because the token is a self-contained, signed JWT, the bot manager performs standard JOSE/JWK Set verification and needs no per-request callout to a third party.
  Issuer keys SHOULD be cached.

* **Access Control by Identity.**
  A validated token gives the bot manager verified initiator identity (and, where the request targets an agent, target identity), which it MAY use to admit the request rather than challenging or blocking it as suspected automation.
...
* It SHOULD use the `verifier`/`verified`/`verification_id` sub-claims, and the `amr` {{I-D.skyfire-oauth-amr-values}} and `ivm` {{I-D.skyfire-oauth-id-verification}} claims where present, to weight how the principal was authenticated and verified.
...
In all of these arrangements the token issuer is not in the transaction path: the fraud manager verifies the self-contained token locally by JWKS, as described in {{ProcModel}}.
...
Not all actions carry the same risk: viewing a loyalty balance is far less sensitive than making a large payment.
...
* Where the agent needs to act within an existing account, the token MAY be exchanged for an OAuth access token using OAuth 2.0 Token Exchange {{RFC8693}}: a Security Token Service, Identity Provider, or OAuth 2.0 {{RFC6749}} Authorization Server validates the KYA token, extracts principal claims such as the email address from `hid`, and issues an access token that the agent uses against the Target.
...
* It SHOULD use the OAuth 2.0 Token Exchange pattern {{RFC8693}} to exchange a KYA token for an access token, enabling logged-in experiences (loyalty, saved preferences, offers) for human-via-agent sessions, applying the sender-constraining and cross-domain-trust considerations of {{ATOProt}}.
...
The same granularity supports responses short of a block, such as throttling, requiring step-up ({{StepUp}}), or lowering an assurance score, applied at whichever tier is appropriate.
...
## Non-Payment and Hand-off Flows
...
Intermediaries SHOULD accept KYA-only requests (those carrying a KYA token but no PAY token) for actions that do not themselves settle a payment, and MUST NOT treat the absence of a PAY token as a reason to classify an otherwise valid human-via-agent request as a bot.
...
Consumers observe the result of these choices through the `verified`/`verifier` sub-claims and, where used, the `ivm` {{I-D.skyfire-oauth-id-verification}} and `amr` {{I-D.skyfire-oauth-amr-values}} claims, rather than through any mandated creation flow.
...
KYAPay tokens carry identity and, for PAY tokens, payment credentials.
...
Proof of possession does not prevent this: the malware's requests carry valid bindings and are indistinguishable from the real agent's.
...
Because KYA tokens are JWTs, selective disclosure (for example, SD-JWT) is a forward-compatible path to revealing only the claims a given Target needs.
...
* Comments: Carries one or more KYAPay tokens (JWS Compact Serialization) on an HTTP request.
...
the `amr` extensions and the `ivm` claim are registered by {{I-D.skyfire-oauth-amr-values}} and {{I-D.skyfire-oauth-id-verification}} respectively.
...
The authors thank the contributors to the KYAPay Token {{I-D.skyfire-oauth-kyapay-token}} and the partners in the KYAPay consortium (see {{KYAPAY-ORG}}) -- including bot-management, fraud, CIAM, and ATO vendors -- whose deployment and review experience informed the usage patterns and open issues described here.
