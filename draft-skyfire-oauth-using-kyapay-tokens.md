...
KYAPay tokens replace network-level inference with un-forgeable, issuer-signed assertions. Signed by a trusted issuer and verified against public keys, the token cryptographically asserts the agent instance (aid), the execution platform (apd), and the authorizing human principal (hid). Where network indicators only establish that a request comes from a known operator's network, a KYAPay token establishes that a specific agent is acting for a specific, verified subscriber.
